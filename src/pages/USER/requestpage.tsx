...
const RequestPage: React.FC = () => {
  ...
  const handleApply = async () => {
    if (
      oneCompany?.id &&
      window.confirm(`Send internship application to ${oneCompany.name}?`)
    ) {
  ...
  const handleUpdateStatus = async (
    ...
    if (
      window.confirm(
        `Are you sure you want to ${actionText} ${studentName}'s application?`
      )
    ) {
  ...
  if (!oneCompany) {
    return (
      <div
        className={`min-h-screen ${styles.bg} ${styles.textMain} flex flex-col items-center justify-center`}
      >
      ...
      <div className="max-w-6xl mx-auto px-6 pt-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-6">
          <div className="flex flex-col gap-6">
            ...
            >
              <div
                className={`w-32 h-32 rounded-2xl flex items-center justify-center shrink-0 border overflow-hidden ${isNight ? "bg-[#2a2a2a] border-white/10" : "bg-slate-50 border-slate-200"}`}
              >
                {oneCompany.logo ? (
                  <img
                    src={oneCompany.logo}
              ...
              <div>
                ...
                <div
                  className={`flex items-start gap-2 text-sm ${styles.textSub} mb-4`}
                >
            ...
            >
              <div
                className={`px-6 py-4 border-b flex items-center justify-between ${isNight ? "border-white/5" : "border-slate-200"}`}
              >
                ...
                <span
                  className={`text-xs font-medium px-2.5 py-1 rounded-full ${isNight ? "bg-white/5 text-slate-300" : "bg-slate-100 text-slate-600"}`}
                >
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                  <thead
                    className={`text-xs font-semibold ${styles.textSub} ${isNight ? "bg-[#1a1a1a]" : "bg-slate-50"} border-b ${isNight ? "border-white/5" : "border-slate-200"}`}
                  >
                    <tr>
                    ...
                    {oneCompany.students && oneCompany.students.length > 0 ? (
                        ...
                        return (
                          ...
                          >
                            <td className="px-6 py-4 font-medium flex items-center gap-3">
                              <div
                                className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${isNight ? "bg-blue-500/20 text-blue-400" : "bg-blue-100 text-blue-600"}`}
                              >
                            ...
                            <td
                              className={`px-6 py-4 text-center ${styles.textSub}`}
                            >
                              {item.class || item.major || "-"}
                            </td>
                            <td
                              className={`px-6 py-4 text-center font-mono text-xs ${styles.textSub}`}
                            >
                              {item.nomorInduk}
                            </td>
                            <td className="px-6 py-4 text-center">
                              <span
                                className={`inline-flex items-center justify-center px-2.5 py-1 rounded-md text-[11px] font-bold uppercase tracking-wider border ${getStatusStyle(currentStatus)}`}
                              >
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center justify-center gap-2">
                                  {isPending ? (
                                    ...
                                    <span
                                      className={`text-xs font-medium ${currentStatus === "APPROVED" ? "text-emerald-500" : "text-red-500"}`}
                                    >
                      ...
                      <tr>
                        <td
                          ...
                          className={`px-6 py-12 text-center ${styles.textSub}`}
                        >
          ...
          <div className="flex flex-col gap-6">
            {role === "STUDENT" && (
              ...
              >
                ...
                <p className={`text-sm ${styles.textSub}`}>
                  Submit your application for an internship placement at this
                  company. Make sure your profile is complete.
                </p>
                <button
                  onClick={handleApply}
            ...
            >
              ...
              <div
                className={`w-full h-64 rounded-xl overflow-hidden ${isNight ? "bg-[#2a2a2a]" : "bg-slate-100"} mx-auto mb-1`}
              ...
              >
                {oneCompany.address ? (
                  ...
                  <div
                    className={`w-full h-full flex flex-col items-center justify-center ${styles.textSub}`}
                  >
