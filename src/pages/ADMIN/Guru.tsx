...
import { ArrowLeft, Plus, Trash2, Edit2, Search, ChevronLeft, Loader2, User } from "lucide-react";
...
const totalPages = pagination?.total_pages || (pagination as any)?.totalPages || 1;
const totalItems = pagination?.total_items || (pagination as any)?.totalItems || 0;
...
await deleteUser(id);
showAll(`paginate=true&limit=10&page=${page}&where=roles.some.code:${role.toUpperCase()}`);
}
};
...
>
<Plus size={16} /> Add {role === "teacher" ? "Teacher" : "Student"}
</button>
)}
</div>

{/* SEARCH BAR */}
<div className={`p-4 rounded-xl border ${styles.card} mb-6 flex flex-col sm:flex-row gap-4 items-center justify-between`}>
...
<Loader2 className={`animate-spin ${styles.textSub}`} size={24} />
<span className={`text-sm ${styles.textSub}`}>Loading registry data...</span>
</div>
) : safeListUser.length ? (
...
<div className={`w-10 h-10 rounded-full flex items-center justify-center overflow-hidden shrink-0 ${isNight ? 'bg-[#2a2a2a]' : 'bg-slate-100'}`}>
{item.profilePhoto ? (
<img src={item.profilePhoto} className="w-full h-full object-cover" alt={item.name} />
) : (
<User size={18} className={styles.textSub} />
)}
</div>
<div>
<h3 className="text-base font-semibold leading-tight mb-1">{item.name}</h3>
<div className="flex items-center gap-3 text-xs">
<span className={`${styles.textSub} font-mono`}>{item.nomorInduk || 'No ID'}</span>
{item.major && (
<span className={`px-2 py-0.5 rounded-full font-medium ${isNight ? 'bg-blue-500/10 text-blue-400' : 'bg-blue-50 text-blue-600'}`}>
{item.major}
</span>
)}
</div>
...
<>
<button 
onClick={() => navigate(`${listed.editguru}?id=${item.id}`)} 
...
<span className={`text-[10px] font-medium uppercase tracking-widest ${styles.textSub} opacity-50`}>
Read Only
</span>
)}
</div>
...
<div className={`py-20 rounded-xl border ${styles.card} flex flex-col items-center justify-center text-center`}>
<Search size={32} className={`mb-3 ${styles.textSub} opacity-50`} />
<h3 className="text-base font-medium">No records found</h3>
<p className={`text-sm mt-1 ${styles.textSub}`}>Try adjusting your search query.</p>
</div>
)}
</AnimatePresence>
</div>
...
<div className={`mt-8 flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t ${isNight ? 'border-white/10' : 'border-slate-200'}`}>
