    ...
    return (
        <div data-theme={theme} className="min-h-screen bg-base-200">
            ...
            <div className="max-w-6xl mx-auto px-6 py-8">
                <button onClick={() => navigate(-1)} className="mb-5 flex items-center gap-2 text-sm font-bold text-gray-500 hover:underline">
                        ...
                        students.map((item) => (
                            <div key={item.id} className="flex items-center gap-4 px-5 py-4 border-b last:border-none hover:bg-base-200 transition">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-base-content/60">{item.nomorInduk}</p>
                                </div>
                            </div>
                        ))
                    ) : (
                        <div className="p-10 text-center text-gray-400">Belum ada data siswa.</div>
                    )}
                </div>
            </div>
        </div>
