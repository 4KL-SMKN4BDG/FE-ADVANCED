...
import { ArrowLeft, Plus, Building2, Trash2, Edit2, Send, ChevronRight, Loader2, Search } from "lucide-react";
...
<span className={`text-sm ${styles.textSub}`}>Loading company data...</span>
</div>
) : company?.length === 0 ? (
<div className={`py-20 rounded-xl border ${styles.card} flex flex-col items-center justify-center text-center`}>
<Building2 size={32} className={`mb-3 ${styles.textSub} opacity-50`} />
<h3 className="text-base font-medium">No companies registered</h3>
<p className={`text-sm mt-1 ${styles.textSub}`}>Currently there are no partner companies in the system.</p>
</div>
) : (
...
{/* Company Logo */}
<div className={`w-14 h-14 rounded-xl flex items-center justify-center overflow-hidden shrink-0 border ${isNight ? 'border-white/5 bg-slate-800' : 'border-slate-100 bg-white'}`}>
{item?.logo ? (
...
<p className={`text-sm ${styles.textSub} line-clamp-2 max-w-2xl`}>
{item.description || "No description provided for this company."}
</p>
</div>
</div>
...
<button 
onClick={() => navigate(`${listed.RequestPage}?id=${item.id}`)} 
...
<>
<button 
onClick={() => navigate(`${listed.AddPerusahaan}?id=${item.id}`)} 
