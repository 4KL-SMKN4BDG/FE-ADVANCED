...
const DashboardAdmin = () => {
  ...
  return (
    <div
      className={`min-h-screen ${styles.bg} ${styles.textMain} transition-colors duration-300 font-sans pb-20 text-left`}
    >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
          {visibleCards.map((card, i) => (
            ...
            >
              {/* Ikon */}
              <div
                className={`w-12 h-12 rounded-xl mb-auto flex items-center justify-center ${isNight ? "bg-white/5" : "bg-slate-50"}`}
              >
                {card.icon}
              </div>
