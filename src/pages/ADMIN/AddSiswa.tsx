...
card: isNight ? "bg-[#171717] border-white/10" : "bg-white border-slate-200",
...
<button 
onClick={() => navigate(`${listed.UserPage}?role=${roleSearch.toLowerCase()}`)} 
...
<div className="mb-8">
<h1 className="text-3xl font-bold tracking-tight mb-2">Add {roleSearch}s</h1>
<p className={`text-sm ${styles.textSub}`}>
Enter names and identification numbers line by line. Ensure both lists match in length.
</p>
</div>
...
{/* Input Nama */}
<motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`p-6 rounded-xl border ${styles.card}`}>
...
required 
/>
<div className={`mt-3 text-xs font-medium ${styles.textSub} flex justify-between`}>
<span>Format: 1 name per line</span>
<span>Rows: {nameInput.split('\n').filter(x => x.trim()).length}</span>
</div>
</motion.div>

{/* Input ID */}
<motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className={`p-6 rounded-xl border ${styles.card}`}>
<div className="flex items-center gap-2 mb-4 font-semibold">
