...
import { ArrowLeft, Save, User, Mail, Hash, BookOpen, Camera, GraduationCap } from "lucide-react";
...
useEffect(() => { if (idUser) showOne(idUser); }, [idUser]);
...
<div className="mb-8">
<h1 className="text-3xl font-bold tracking-tight mb-2">Edit {roleLabel}</h1>
<p className={`text-sm ${styles.textSub}`}>
Update the profile information below.
</p>
</div>

<form onSubmit={handleSubmit} className={`p-8 rounded-xl border ${styles.card} grid grid-cols-1 md:grid-cols-[200px_1fr] gap-10`}>
{/* FOTO UPLOAD */}
<div className="flex flex-col gap-2">
<label className="text-sm font-medium">Profile Photo</label>
<div className={`relative w-full aspect-square rounded-2xl border-2 border-dashed ${styles.input} flex items-center justify-center overflow-hidden hover:border-blue-500 transition-colors group`}>
{formData.preview ? (
...
if(file) setFormData({...formData, profilePhoto: file, preview: URL.createObjectURL(file)})
}} 
...
<label className="text-sm font-medium flex items-center gap-2"><User size={14}/> Full Name</label>
<input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className={`w-full p-2.5 text-sm rounded-lg border ${styles.input} outline-none`} required />
</div>

<div className="flex flex-col gap-1.5">
<label className="text-sm font-medium flex items-center gap-2"><Hash size={14}/> {isStudent ? 'NIS' : 'NIP'} Number</label>
<input type="text" value={formData.nomorInduk} onChange={(e) => setFormData({...formData, nomorInduk: e.target.value})} className={`w-full p-2.5 text-sm font-mono rounded-lg border ${styles.input} outline-none`} required />
</div>

<div className="flex flex-col gap-1.5">
<label className="text-sm font-medium flex items-center gap-2"><Mail size={14}/> Email Address</label>
<input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className={`w-full p-2.5 text-sm rounded-lg border ${styles.input} outline-none`} />
</div>

<div className="flex flex-col gap-1.5 sm:col-span-2">
<label className="text-sm font-medium flex items-center gap-2"><BookOpen size={14}/> {isStudent ? 'Major / Class' : 'Department'}</label>
<input type="text" value={formData.major} onChange={(e) => setFormData({...formData, major: e.target.value})} className={`w-full p-2.5 text-sm rounded-lg border ${styles.input} outline-none`} />
</div>
